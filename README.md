# Efficient algorithms for Front-Door Adjustment
  ...
  Internally, the functions ```findfrontdoor``` and ```enumeratefrontdoor``` in the file ```frontdoor.jl``` are called, which are implementations of the algorithms presented in the paper: Finding Front-Door Adjustment Sets in Linear Time; Marcel Wienöbst, Benito van der Zander, Maciej Liskiewicz (available on arXiv under https://arxiv.org/abs/2211.16468). The time complexity of ```findfrontdoor``` is $O(n+m)$ and ```enumeratefrontdoor``` has delay $O(n(n+m))$. 
